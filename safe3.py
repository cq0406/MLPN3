...
if st.session_state.show_training:
    ...
    model_exists = os.path.exists("pk_model.pkl") or os.path.exists("outcome_model.pkl")
    ...
    # ==================== 云端模式：仅展示已有模型性能 ====================
    if is_cloud and model_exists:
        st.warning("🔒 云端部署环境：模型已内置，不支持重新训练。")
        info = {}
        ...
        if info:
            ...
            with col1:
                ...
                if "pk_r2" in info:
                    st.metric("PK模型测试集 R²", f"{info['pk_r2']}")
                st.caption(f"训练样本数：{info.get('pk_samples', '--')}")
            with col2:
                if "cls_acc" in info:
                    st.metric("血药浓度达标预测准确率", f"{info['cls_acc']:.1%}")
    ...
    with col_tr1:
        if st.button("训练PK预测模型（保存至桌面）", key="train_pk_stable"):
            with st.spinner("训练中..."):
                ...
                info = {"pk_mae": mae, "pk_r2": r2, "pk_samples": len(X_train)}
                if os.path.exists(os.path.join(desktop, "model_info.json")):
                    try:
                        with open(os.path.join(desktop, "model_info.json"), "r") as f:
                            old_info = json.load(f)
                    except:
                        old_info = {}
                    old_info.update(info)
                    info = old_info
                with open(os.path.join(desktop, "model_info.json"), "w") as f:
        ...
        if can_train_cls:
            if st.button("训练血药浓度达标预测模型（保存至桌面）", key="train_cls_stable"):
                with st.spinner("训练中..."):
                    ...
                    info = {"cls_acc": acc, "cls_samples": len(X_train)}
                    if os.path.exists(os.path.join(desktop, "model_info.json")):
                        try:
                            with open(os.path.join(desktop, "model_info.json"), "r") as f:
                                old_info = json.load(f)
    ...
    if "pk_model" in st.session_state:
        with st.expander("View PK Model Feature Importance"):
            ...
            imp_df = pd.DataFrame({"Feature": eng_features, "Importance": importances}).sort_values("Importance", ascending=False)
            threshold = st.slider("Importance threshold", 0.0, float(imp_df["Importance"].max()), 0.02, 0.005, key="pk_local")
    ...
    if "outcome_model" in st.session_state:
        with st.expander("View Attainment Model Feature Importance"):
            ...
            threshold_cls = st.slider("Importance threshold", 0.0, float(imp_df_cls["Importance"].max()), 0.02, 0.005, key="cls_local")
            ...
            ax.set_xlabel("Importance")
            ax.set_title("Attainment Model Feature Importance (Top 15)")
            ax.spines['top'].set_visible(False)
            ax.spines['right'].set_visible(False)
            ax.legend()
            st.pyplot(fig)
