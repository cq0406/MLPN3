...
if st.session_state.show_training:
    ...
    # 云端权限控制：若存在模型文件，则仅展示模型性能，禁止训练和查看特征重要性
    ...
    if len(df) < 10:
        ...
        if "给药前0.5h浓度" not in df.columns or "结论类型" not in df.columns:
            ...
            if len(X) < 10:
                ...
                with col_tr1:
                    if st.button("训练PK预测模型（保存至桌面）", key="train_pk_stable"):
                        with st.spinner("训练中，请稍候..."):
                            ...
                            mae_pk = mean_absolute_error(y_reg_test, pred_reg)
                            st.success(f"模型训练完成！测试集 MAE: {mae_pk:.2f} mg/L")

                            # 保存模型到桌面
                            pk_path = os.path.join(desktop, "pk_model.pkl")
                            feat_path = os.path.join(desktop, "pk_features.pkl")
                            joblib.dump(model_reg, pk_path)
                            joblib.dump(all_feature_cols, feat_path)
                            st.info(f"模型已保存至桌面：{pk_path}、{feat_path}")
                ...
                with col_tr2:
                    if can_train_cls:
                        if st.button(f"训练疗效分类模型（保存至桌面）", key="train_cls_stable"):
                            with st.spinner("训练中，请稍候..."):
                                ...
                                acc_cls = accuracy_score(y_cls_test, pred_cls)
                                st.success(f"模型训练完成！测试集准确率: {acc_cls:.2%}")

                                # 保存模型到桌面
                                cls_path = os.path.join(desktop, "outcome_model.pkl")
                                enc_path = os.path.join(desktop, "outcome_encoder.pkl")
                                joblib.dump(model_cls, cls_path)
                                joblib.dump(le_outcome, enc_path)
                                st.info(f"模型已保存至桌面：{cls_path}、{enc_path}")
