...
if st.session_state.show_training:
    import os
    # 检测是否在 Streamlit Cloud 环境中
    if os.path.exists("/mount/src/"):
        # 若模型文件已上传，则禁用训练，仅展示特征重要性
        if os.path.exists("pk_model.pkl") or os.path.exists("outcome_model.pkl"):
            st.warning("🔒 云端部署环境：模型已内置，不支持重新训练。如需更新模型，请本地训练后重新上传。")
            # 显示已有模型的特征重要性（如果有）
            if "pk_model" in st.session_state:
                with st.expander("查看PK模型特征重要性"):
                    # 此处可复制之前的特征重要性展示代码，或直接跳过
                    pass
            st.stop()
